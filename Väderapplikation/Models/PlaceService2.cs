...
namespace Väderapplikation.Models
{
    ...
    {       
        ...
        public ExtWeather GetWeatherInfo(string place, string region)
        {
            ...
            var request = WebRequest.Create(requestUristring);
            List<double> informationdouble = new List<double>(); //Temperatur
            List<DateTime> informationdate = new List<DateTime>();//Datum
            List<string> informationweather = new List<string>();//Väder
                    ...
                    foreach (XElement element in timelist)
                    {
                        if (element.Attribute("period").Value == "2" || element.Attribute("period").Value == "3")
                        {
                            if (element.Attribute("from").Value.Substring(0, 10) != tmpDate)
                            {
                                ...
                                {
                                    //Syftar alltså till temperaturen. 
        ...
        /* YR-koordinaterna stämmer inte. 
        public List<string> GetYrCoordinates(string place, string region)
        {
            List<string> coordinates = new List<string>();
            string requestUristring = String.Format(@"http://www.yr.no/place/Sweden/{0}/{1}/forecast.xml", region, place);
            var request = WebRequest.Create(requestUristring);
            using (var response = request.GetResponse())
            {
                ...
                {
                    var document = XDocument.Load(stream);
                    var weatherdata = document.Descendants("weatherdata")
                        .Take(1)
                        .ToList();
                    var locationlist = weatherdata.Descendants("location")
                            .Take(1)
                            .ToList();
                    var newlocation = locationlist[0];
                    var newlocationlocationlist = newlocation.Descendants("location")
                        .Take(1)
                        .ToList();
                    var newlocationlocation = newlocationlocationlist[0];
                    var testattribute = newlocationlocation.FirstAttribute;
                    var testattributevalue = newlocationlocation.FirstAttribute.Value;
                    var newtestattribute = newlocationlocation.Attribute("altitude");
                    var newtestattribute2 = newlocationlocation.Attribute("latitude");
                    var newtestattribute3 = newlocationlocation.Attribute("longitude");
                    var newtestattribute2value = newtestattribute2.Value;
                    var newtestattribute3value = newtestattribute3.Value;
                    //var newtestattribute3 = newlocationlocation.Attribute[0];

                    coordinates.Add(newtestattribute2value);
                    coordinates.Add(newtestattribute3value);
